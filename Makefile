...
install: all
	...
	@echo "INSTALL .local/bin/nsxiv-opts"
	mkdir -p $(USRBIN)
	cp scripts/nsxiv-opts $(USRBIN)/
	chmod 755 $(USRBIN)
	chown $$SUDO_USER:wheel $(USRBIN)/nsxiv-opts
...
uninstall: uninstall-icon
	...
	@echo "REMOVE .local/bin/nsxiv-opts"
	rm -f $(USRBIN)/nsxiv-opts
	@echo "REMOVE nsxiv.1"
